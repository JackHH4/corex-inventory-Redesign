...
local function NormalizePlate(plate)
    ...
    -- strip all whitespace so "LH BD3P" matches "LHBD3P" / padded GTA plates
...
local function IsPortableVehicleModel(catalogId, model)
    ...
    if catalog ~= nil and vehicleDef ~= nil then
        return true
    end

    -- vehicleshop / custom: metadata model is enough when catalog is not in corex-core
    local requireMatch = Config.PortableVehicles and Config.PortableVehicles.RequireCatalogMatch == true
    if requireMatch then
        return false
    end

    local modelKey = NormalizeVehicleKey(model)
    return modelKey ~= nil and modelKey ~= ''
end
...
local function IsPortableVehicleModelHash(catalogId, modelHash)
    ...
    if catalog and type(catalog.vehicles) == 'table' then
        for _, vehicle in ipairs(catalog.vehicles) do
            local m = vehicle.model
    ...
    -- Allow finalize when model is not in core catalog (vehicleshop purchased cars)
    local requireMatch = Config.PortableVehicles and Config.PortableVehicles.RequireCatalogMatch == true
    if requireMatch then
        return false
    end

    return true, nil, nil, catalog
end
...
local function EnsureItemMetadata(itemName, metadata)
    ...
    -- Portable / owned vehicles: human-readable label + description (never stack)
    local lower = type(itemName) == 'string' and string.lower(itemName) or ''
    if lower == 'portable_vehicle' or lower == 'rental_bicycle' then
        local model = safeMetadata.model
        local label = safeMetadata.label
        local plate = safeMetadata.plate
        local owner = safeMetadata.owner

        if (not label or label == '' or label == 'Vehicle') and type(model) == 'string' and model ~= '' then
            -- Title-case model name for display (zr380 -> Zr380)
            label = model:gsub('^%l', string.upper)
        ...
        if type(plate) == 'string' and plate ~= '' then
            parts[#parts + 1] = ('Plate: %s'):format(plate)
        end
        if type(owner) == 'string' and owner ~= '' then
            local shortOwner = owner
            if #shortOwner > 18 then
                shortOwner = shortOwner:sub(1, 12) .. '…' .. shortOwner:sub(-4)
            end
            parts[#parts + 1] = ('Owner: %s'):format(shortOwner)
        end
        if type(model) == 'string' and model ~= '' then
            parts[#parts + 1] = ('Model: %s'):format(model)
        end

        if #parts > 0 then
            safeMetadata.description = table.concat(parts, ' | ')
        end

        -- Unique key so nothing ever merges these stacks
        if not safeMetadata.uid then
            safeMetadata.uid = ('%s_%s_%s'):format(
                tostring(model or 'veh'),
                tostring(plate or 'plate'),
                tostring(os.time())
            )
        end
    end
...
local function AddItem(src, itemName, count, metadata, x, y)
    ...
    -- Stack: never merge unique metadata items (vehicles, weapons with serial, etc.)
    local lowerName = string.lower(itemName)
    local isUniqueItem = lowerName == 'portable_vehicle' or lowerName == 'rental_bicycle'
        or (metadata and (metadata.uid or metadata.plate or metadata.serial or metadata.unique == true))

    local canStack = (data.stackable == true) or (data.stackable ~= false and not GetWeaponDefinition(itemName) and not isUniqueItem)
    if data.stackable == false then canStack = false end

    if canStack and not isUniqueItem then
        local maxStack = tonumber(data.maxStack)
        if not maxStack or maxStack < 1 then maxStack = 999999999 end
        local remain = count
        for _, item in ipairs(inv.items) do
            if remain <= 0 then break end
            local same = item.name == itemName or (type(item.name) == 'string' and string.lower(item.name) == lowerName)
            if same then
                local md = item.metadata
                if not (md and (md.uid or md.plate or md.serial or md.unique == true)) then
                    local cur = tonumber(item.count) or 0
                    local space = maxStack - cur
                    if space > 0 then
                        local add = math.min(space, remain)
                        item.count = cur + add
                        remain = remain - add
                    end
                end
        ...
        if remain < count then
            local added = count - remain
            inv.weight = inv.weight + (data.weight * added)
            if remain <= 0 then
                SaveInventory(src)
                TriggerClientEvent('corex-inventory:client:update', src, inv)
                return true
...
RegisterNetEvent('corex-inventory:server:open', function()
    ...
    if inv then
        if CompactStacks then CompactStacks(src) end
        inv = Inventories[src] or inv
...
local function ValidateAndApplyCompactLayout(inv, proposed)
    ...
    local posBySlot = {}
    local seen = {}
    local gridW = Config.GridWidth or 8
    local gridH = Config.GridHeight or 10
    ...
    for _, entry in ipairs(proposed) do
        if type(entry) == 'table' then
            ...
            if slotId ~= nil then
                ...
                if #slotId > 0 and #slotId <= 64 and x and y and x == x and y == y then
                    x = math.floor(x)
                    y = math.floor(y)
    ...
    for _, item in ipairs(inv.items) do
        ...
        if x < 1 or y < 1 or (x + w - 1) > gridW or (y + h - 1) > gridH then
            return false
        end
...
RegisterNetEvent('corex-inventory:server:compact', function(positions)
    ...
    if TrySetBusy and not TrySetBusy(src) then
    ...
    local inv = Inventories[src]
    if not inv then
        if ClearBusy then ClearBusy(src) end
        return
    end
...
RegisterNetEvent('corex-inventory:server:requestAmmoReload', function(slotId, weaponName)
    ...
    local currentAmmo = math.max(0, math.floor(tonumber(weaponItem.metadata.ammo) or 0))
    local clipSize = math.max(1, math.floor(tonumber(weaponDef.clipSize) or 30))

    if currentAmmo >= clipSize then
        TriggerClientEvent('corex-inventory:client:ammoReloadResult', src, false, slotId, canonicalName, currentAmmo, 0, 'Magazine is full')
        return
    end

    local need = clipSize - currentAmmo
    local have = GetInventoryItemCount(inv, weaponDef.ammoType) or 0
    if have < 1 then
        TriggerClientEvent('corex-inventory:client:ammoReloadResult', src, false, slotId, canonicalName, currentAmmo, 0, 'No ammo item available')
        return
    end

    -- ox-style: each ammo item = 1 round (only fill what fits in the clip)
    local take = math.min(need, have)
    if not RemoveItem(src, weaponDef.ammoType, take) then
        TriggerClientEvent('corex-inventory:client:ammoReloadResult', src, false, slotId, canonicalName, currentAmmo, 0, 'Failed to consume ammo item')
        return
    end

    local newAmmo = currentAmmo + take
    weaponItem.metadata.ammo = newAmmo
    SaveInventory(src, true)

    TriggerClientEvent('corex-inventory:client:ammoReloadResult', src, true, slotId, canonicalName, newAmmo, take, weaponDef.ammoType)
end)
...
RegisterNetEvent('corex-inventory:server:finalizePortableVehicle', function(netId)
    ...
    if not isAllowed then
        Debug('Warn', ('finalizePortable: model not allowed (src=%d catalog=%s)'):format(src, tostring(catalogId)))
        return
    end

    local plateText = NormalizePlate(GetVehicleNumberPlateText(entity))
    local pendingPlate = NormalizePlate(pending.plate)
    -- Prefer pending plate (source of truth from item metadata)
    if pendingPlate ~= '' and not PlatesMatch(pendingPlate, plateText) then
        Debug('Warn', ('finalizePortable: plate mismatch pending=%s entity=%s — using pending'):format(
            tostring(pendingPlate), tostring(plateText)
        ))
        plateText = pendingPlate
    elseif pendingPlate ~= '' then
        plateText = pendingPlate
    end

    local modelKey = NormalizeVehicleKey(pending.model or modelFromHash)
    if not modelKey then
        Debug('Warn', 'finalizePortable: missing modelKey')
        return
    end
    -- Soft model check: entity hash should match pending model when possible
    local entityHash = GetEntityModel(entity)
    if entityHash ~= GetHashKey(modelKey) then
        Debug('Warn', ('finalizePortable: model hash mismatch entity=%s pending=%s — still registering'):format(
            tostring(entityHash), tostring(modelKey)
        ))
    end
...
RegisterNetEvent('corex-inventory:server:pickupPortableVehicle', function(netId, props)
    local src = source
    ...
    -- Persist vehicle modifications (mods, extras, colours, neon, etc.)
    if type(props) == 'table' then
        -- keep payload compact but complete
        meta.props = props
        meta.hasMods = true
    end
...
local function DeployPortableVehicleFromItem(src, slotId, requestedItemName)
    ...
    if not model or model == '' or plate == '' then
        TriggerClientEvent('corex-inventory:client:portableVehicleNotify', src, 'This vehicle item is invalid (missing model/plate).', 'error')
        return
    end

    -- Optional catalog whitelist (off by default so vehicleshop models like zr380 work)
    if Config.PortableVehicles and Config.PortableVehicles.RequireCatalogMatch == true then
        if not IsPortableVehicleModel(catalogId, model) then
    ...
    local savedProps = meta.props
    if type(savedProps) ~= 'table' then savedProps = nil end
    ...
    CreateThread(function()
        Wait((Config.PortableVehicles and Config.PortableVehicles.DeployTimeout) or 30000)
...
RegisterNetEvent('corex-inventory:server:deployPortableVehicleFromItem', function(slotId, requestedItemName)
    ...
    Debug('Info', ('Portable vehicle deploy requested by %d (slot %s, item %s)'):format(src, tostring(slotId), tostring(requestedItemName)))
    DeployPortableVehicleFromItem(src, slotId, requestedItemName)
end)
...
RegisterCommand('clearinv', function(src, args)
    ...
    if args[1] and tonumber(args[1]) then
        -- admin-style: /clearinv [id] — only if source is 0 (console) or has ace
        if src == 0 or IsPlayerAceAllowed(src, 'command.clearinv') then
            target = tonumber(args[1])
        end
    end
    if not target or target < 1 then
        if src > 0 then
            TriggerClientEvent('chat:addMessage', src, { args = { '^1SYSTEM', 'Invalid player' } })
        end
        return
    end
    ...
    if not inv then
        if src > 0 then
            TriggerClientEvent('chat:addMessage', src, { args = { '^1SYSTEM', 'Inventory not loaded' } })
        end
        return
    end
    inv.items = {}
    inv.weight = 0
    SaveInventory(target, true)
    TriggerClientEvent('corex-inventory:client:update', target, inv)
    if src > 0 then
        TriggerClientEvent('chat:addMessage', src, { args = { '^2SYSTEM', 'Inventory cleared' } })
    else
        print(('[corex-inventory] Cleared inventory for %d'):format(target))
    end
end, false)



local function CanStacksMerge(aName, aMeta, bName, bMeta, itemData)
    if not aName or not bName or not itemData then return false end
    if string.lower(tostring(aName)) ~= string.lower(tostring(bName)) then return false end
    if itemData.stackable == false then return false end
    local function isUnique(md)
        if type(md) ~= 'table' then return false end
        return md.uid ~= nil or md.plate ~= nil or md.serial ~= nil or md.unique == true
    end
    if isUnique(aMeta) or isUnique(bMeta) then return false end
    return true
end

local function MergePlayerItems(src, fromSlot, toSlot, amount)
    local inv = Inventories[src]
    if not inv then return false, 'No inventory' end
    local fromItem, fromIdx = FindInventoryItem(inv, nil, fromSlot)
    local toItem, toIdx = FindInventoryItem(inv, nil, toSlot)
    if not fromItem or not toItem then return false, 'Item missing' end
    if tostring(fromItem.slot) == tostring(toItem.slot) then return false, 'Same slot' end
    local data = GetItemData(fromItem.name)
    if not data or not CanStacksMerge(fromItem.name, fromItem.metadata, toItem.name, toItem.metadata, data) then
        return false, 'Cannot stack'
    end
    local maxStack = tonumber(data.maxStack) or 999999999
    if maxStack < 1 then maxStack = 999999999 end
    local fromCount = tonumber(fromItem.count) or 1
    amount = tonumber(amount) or fromCount
    if amount > fromCount then amount = fromCount end
    local toCount = tonumber(toItem.count) or 1
    local space = maxStack - toCount
    if space <= 0 then return false, 'Stack full' end
    local move = math.min(space, amount)
    toItem.count = toCount + move
    fromItem.count = fromCount - move
    if fromItem.count <= 0 then table.remove(inv.items, fromIdx) end
    SaveInventory(src)
    TriggerClientEvent('corex-inventory:client:update', src, inv)
    return true
end

---Merge all stackable duplicates then pack is left to client compact
function CompactStacks(src)
    local inv = Inventories[src]
    ...
    while i <= #inv.items do
        local a = inv.items[i]
        local data = GetItemData(a.name)
        if data and data.stackable ~= false then
            local maxStack = tonumber(data.maxStack) or 999999999
            if maxStack < 1 then maxStack = 999999999 end
            local j = i + 1
            while j <= #inv.items do
                local b = inv.items[j]
                if CanStacksMerge(a.name, a.metadata, b.name, b.metadata, data) then
                    local aCount = tonumber(a.count) or 1
                    local bCount = tonumber(b.count) or 1
                    local space = maxStack - aCount
                    if space > 0 then
                        local move = math.min(space, bCount)
                        a.count = aCount + move
                        b.count = bCount - move
                        changed = true
                        if b.count <= 0 then
                            table.remove(inv.items, j)
                        else
                            j = j + 1
                        end
                    else
...
RegisterNetEvent('corex-inventory:server:mergeItem', function(fromSlot, toSlot, amount)
    local src = source
    MergePlayerItems(src, fromSlot, toSlot, amount)
end)

RegisterNetEvent('corex-inventory:server:sortAndStack', function()
    local src = source
    CompactStacks(src)
    -- After merge, pack items left-to-right top-to-bottom by name
    local inv = Inventories[src]
    if not inv or not inv.items then return end
    -- Sort: largest slot footprint first, then count, then name
    table.sort(inv.items, function(a, b)
        local da = GetItemData(a.name)
        local db = GetItemData(b.name)
        local aw = (da and da.size and da.size.w) or 1
        local ah = (da and da.size and da.size.h) or 1
        local bw = (db and db.size and db.size.w) or 1
        local bh = (db and db.size and db.size.h) or 1
        local aa, ba = aw * ah, bw * bh
        if aa ~= ba then return aa > ba end
        if aw ~= bw then return aw > bw end
        if ah ~= bh then return ah > bh end
        local ca = tonumber(a.count) or 0
        local cb = tonumber(b.count) or 0
        if ca ~= cb then return ca > cb end
        return string.lower(tostring(a.name or '')) < string.lower(tostring(b.name or ''))
    end)
    -- Reflow positions on grid
    local gridW = (inv.grid and inv.grid.w) or (Config.GridWidth or 10)
    local gridH = (inv.grid and inv.grid.h) or (Config.GridHeight or 8)
    local occupied = {}
    local function key(x, y) return x .. ':' .. y end
    local function fits(x, y, w, h)
        if x < 1 or y < 1 or x + w - 1 > gridW or y + h - 1 > gridH then return false end
        for yy = y, y + h - 1 do
            for xx = x, x + w - 1 do
                if occupied[key(xx, yy)] then return false end
            end
        end
        return true
    end
