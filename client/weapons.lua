...
-- ox-style: keep empty weapons equipped, no native auto-reload/swap
CreateThread(function()
    while true do
...
RegisterNetEvent('corex-inventory:client:ammoReloadResult', function(success, slotId, weaponName, newAmmo, addedAmmo, extra)
    ...
    if not HasPedGotWeapon(ped, hash, false) then
        GiveWeaponToPed(ped, hash, safeAmmo, false, true)
    end
    SetPedAmmo(ped, hash, safeAmmo)
    SetCurrentPedWeapon(ped, hash, true)
    -- trigger native reload anim when possible (ox-like feedback)
    if IsPedWeaponReadyToShoot(ped) or true then
        MakePedReload(ped)
    end
    isReloadingWeapon = false

    Corex.Functions.Notify(('Reloaded +%s (clip %s)'):format(tostring(addedAmmo or 0), tostring(safeAmmo)), 'success', 2000)
end)
    ...
    while true do
        ...
        if cachedPed then
            ...
            if weaponHash ~= unarmedHash then
                ...
                -- ox-style: ammo empty must NOT strip the inventory weapon.
                -- If GTA forced unarmed, put the equipped weapon back with current metadata ammo (can be 0).
                if equippedWeapon and equippedWeapon ~= 'UNKNOWN_WEAPON' and Weapons[equippedWeapon] and Weapons[equippedWeapon].ammoType then
                    local hash = GetHashKey(equippedWeapon)
                    local keepAmmo = 0
                    if equippedWeaponData and equippedWeaponData.ammo ~= nil then
                        keepAmmo = SafeAmmo(equippedWeaponData.ammo)
                    end
                    if not HasPedGotWeapon(cachedPed, hash, false) then
                        GiveWeaponToPed(cachedPed, hash, keepAmmo, false, true)
                    end
                    SetPedAmmo(cachedPed, hash, keepAmmo)
                    SetCurrentPedWeapon(cachedPed, hash, true)
                    currentWeaponHash = hash
                    lastAmmoCount = keepAmmo
                    Wait(0)
                else
                    if equippedWeapon then
                        currentRecoil.x = 0
                    ...
                    -- melee / unknown: allow holster when native goes unarmed
                    if not equippedWeapon or not (Weapons[equippedWeapon] and Weapons[equippedWeapon].ammoType) then
                        ClearEquippedWeaponState()
