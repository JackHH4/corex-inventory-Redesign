...
local function UseQuickSlot(slotNum)
    ...
    local hotkeyMapping = {}
    local gridCols = Config.GridWidth or 10
    local gridRows = Config.GridHeight or 8
...
RegisterNetEvent('corex-inventory:client:openLootContainer', function(containerId, items, containerLabel, revealDelay)
    ...
    local inventoryData = {
        ...
        weight = currentWeight,
        maxWeight = Config.MaxWeight or 200,
        grid = {w = Config.GridWidth or 10, h = Config.GridHeight or 8},
        itemsData = allItems,
        groundItems = {},
        isContainer = true
    }
...
RegisterNUICallback('takeLootItem', function(data, cb)
    ...
    if type(containerId) ~= 'string' or not itemIndex then return end
    itemIndex = math.floor(itemIndex)
    if itemIndex < 1 then return end
    -- Keep container session open; do NOT close inventory/loot here
    TriggerServerEvent('corex-loot:server:takeItem', containerId, itemIndex)
end)
...
RegisterNetEvent('corex-inventory:client:lootItemTaken', function(itemIndex)
    SendNUIMessage({
        action = 'lootItemTaken',
        itemIndex = itemIndex
    })
end)

RegisterNetEvent('corex-loot:client:takeResult', function(success, itemIndex, errorMsg)
    -- Mirror takeResult into NUI so we only remove the tile after server confirms
    SendNUIMessage({
        action = 'lootTakeResult',
...
RegisterNUICallback('mergeItem', function(data, cb)
    cb('ok')
    if type(data) ~= 'table' then return end
    TriggerServerEvent('corex-inventory:server:mergeItem', data.fromSlot or data.sourceSlot, data.toSlot or data.targetSlot, data.count or data.amount)
end)

RegisterNUICallback('mergeGroundItem', function(data, cb)
    cb('ok')
    if type(data) ~= 'table' then return end
    TriggerServerEvent('corex-inventory:server:pickupItem', data.groundSlot or data.dropId, data.x, data.y)
end)
