...
function getItemDisplayLabel(item, itemDef) {
    const meta = (item && item.metadata) || {};
    if (meta.label && String(meta.label).trim()) return String(meta.label).trim();
    if (item && item.label && String(item.label).trim()) return String(item.label).trim();
    if (itemDef && itemDef.label) return itemDef.label;
    return (item && item.name) || 'Item';
}

function getItemDisplayDescription(item, itemDef) {
    const meta = (item && item.metadata) || {};
    if (meta.description && String(meta.description).trim()) return String(meta.description).trim();
    const parts = [];
    if (meta.plate) parts.push('Plate: ' + meta.plate);
    if (meta.owner) {
        let o = String(meta.owner);
        if (o.length > 20) o = o.slice(0, 12) + '…' + o.slice(-4);
        parts.push('Owner: ' + o);
    }
    if (meta.model) parts.push('Model: ' + meta.model);
    if (parts.length) return parts.join(' | ');
    if (itemDef && itemDef.description) return itemDef.description;
    return '';
}
...
function updateItemInfo(item) {
    ...
    itemInfoTitle.textContent = getItemDisplayLabel(item, itemDef);
...
function getItemSlotAreaFromEl(el) {
    const step = CONFIG.cellSize + CONFIG.gapSize;
    const w = Math.max(1, Math.round((parseInt(el.style.width, 10) + CONFIG.gapSize) / step));
    const h = Math.max(1, Math.round((parseInt(el.style.height, 10) + CONFIG.gapSize) / step));
    return { w, h, area: w * h };
...
function applySortFallback(mode) {
    ...
    if (mode === 'default') {
        // pack only (keep relative order by current area large→small for denser fit)
        els.sort((a, b) => getItemSlotAreaFromEl(b).area - getItemSlotAreaFromEl(a).area);
    ...
    els.sort((a, b) => {
        ...
        // size / stack mode: largest slot footprint first, then count, then name
        const A = getItemSlotAreaFromEl(a);
        const B = getItemSlotAreaFromEl(b);
        if (B.area !== A.area) return B.area - A.area;
        if (B.w !== A.w) return B.w - A.w;
        if (B.h !== A.h) return B.h - A.h;
        const ca = parseInt(a.dataset.count || '1', 10);
        const cb = parseInt(b.dataset.count || '1', 10);
...
function wireSortFallback() {
    ...
    buttons.forEach((btn) => {
        ...
        btn.addEventListener('click', (e) => {
            ...
            if (mode === 'stack') {
                // Reference: bars icon = sort + stack all stackable items on server
                fetch(`https://${GetParentResourceName()}/sortAndStack`, {
                    method: 'POST',
                    body: JSON.stringify({})
                }).catch(() => {});
                return;
...
function createItemElement(item, source, quickslotNum) {
    ...
    nameEl.textContent = getItemDisplayLabel(item, itemDef);
    ...
    if (meta.plate) {
        metaBadges.push({ icon: 'fa-car', value: String(meta.plate), cls: 'meta-plate', title: 'Plate: ' + meta.plate });
    }
    ...
    el.dataset.slot = item.slot;
    el.dataset.name = item.name;
    el.dataset.label = getItemDisplayLabel(item, itemDef);
    el.dataset.description = getItemDisplayDescription(item, itemDef);
    el.dataset.count = item.count || 1;
    ...
    if (source === 'ground') {
        el.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            if (el.dataset.taken === 'true' || el.dataset.pending === 'true') return;
            if (inventoryData && (inventoryData.isShop || inventoryData.isContainer)) return;
            el.dataset.pending = 'true';
            const size = (typeof getItemSize === 'function') ? getItemSize(item) : { w: 1, h: 1 };
            let placeX = 1, placeY = 1, found = false;
            const items = (inventoryData && inventoryData.items) || [];
            outer: for (let y = 1; y <= CONFIG.gridRows; y++) {
                for (let x = 1; x <= CONFIG.gridCols; x++) {
                    if (typeof checkCollision === 'function' && !checkCollision(x, y, size.w || 1, size.h || 1, items)) {
                        placeX = x; placeY = y; found = true; break outer;
                    }
                }
            }
            if (!found) { el.dataset.pending = 'false'; return; }
            fetch('https://' + GetParentResourceName() + '/pickupItem', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json; charset=UTF-8' },
                body: JSON.stringify({ groundSlot: item.slot, x: placeX, y: placeY, count: item.count || 1 })
            }).then(() => {
                el.dataset.taken = 'true';
                el.style.opacity = '0';
                setTimeout(() => { if (el.parentNode) el.remove(); }, 200);
            }).catch(() => { el.dataset.pending = 'false'; });
        });
    }
...
function handleDragEnd(e) {
    ...
    } else if (dragSource === 'bag' && playerTarget && playerTarget.type === 'slot') {
        const valid = !checkCollision(playerTarget.x, playerTarget.y, size.w, size.h, inventoryData?.items || []);
        if (valid) {
            fetch('https://' + GetParentResourceName() + '/moveFromBag', {
                ...
                body: JSON.stringify({
                    ...
                    playerX: playerTarget.x,
                    playerY: playerTarget.y
                })
            }).catch(() => {});
        }
    } else if (dragSource === 'bag' && dropSlot) {
...
function showContextMenu(e, item, source, itemEl) {
    ...
    const allowDrop = e.shiftKey === true;
    window.__contextAllowDrop = allowDrop;
    const dropOpt = contextMenu.querySelector('[data-action="drop"]');
    if (dropOpt) dropOpt.style.display = allowDrop ? '' : 'none';

    const itemRect = itemEl.getBoundingClientRect();
    contextMenu.style.left = itemRect.right + 'px';
...
contextMenu.querySelectorAll('.menu-option').forEach(option => {
    option.addEventListener('click', (e) => {
        ...
        if (action === 'use') {
            ...
            if (!selectedItem) return;
            if (!window.__contextAllowDrop) { hideContextMenu(); return; }
...
window.addEventListener('message', (event) => {
    ...
    switch (data.action) {
        ...
        case 'lootTakeResult':
        case 'lootItemTaken': {
            const ok = data.action === 'lootItemTaken' ? true : !!data.success;
            const idx = data.itemIndex != null ? String(data.itemIndex) : null;
            document.querySelectorAll('#drop-grid .item[data-source="container"]').forEach((node) => {
                if (idx && node.dataset.lootIndex !== idx) return;
                if (ok || data.action === 'lootItemTaken') {
                    node.dataset.taken = 'true';
                    node.style.opacity = '0';
                    setTimeout(() => { if (node.parentNode) node.remove(); }, 300);
                } else {
...
/* ========== BAG MODE ========== */
function getBagWeightSegEl() { return document.getElementById('bag-weight-segments'); }
function getBagWeightTextEl() { return document.getElementById('bag-weight-text'); }

function renderBagWeightSegments() {
    const el = getBagWeightSegEl();
    if (!el) return;
    el.innerHTML = '';
...
function updateBagWeightBar(current, max) {
    const el = getBagWeightSegEl();
    if (!el) return;
    const segments = el.querySelectorAll('.weight-segment');
    ...
    const cur = document.getElementById('bag-current-weight');
    const mx = document.getElementById('bag-max-weight');
    if (cur) cur.textContent = Math.round(current * 100) / 100;
    if (mx) mx.textContent = Math.round(max);
}

function checkBagCollision(x, y, w, h, items, excludeSlot) {
    if (x < 1 || y < 1 || x + w - 1 > bagGridCols || y + h - 1 > bagGridRows) return true;
    for (const item of (items || [])) {
        if (excludeSlot != null && String(item.slot) === String(excludeSlot)) continue;
...
function renderBagItems() {
    ...
    const savedCols = CONFIG.gridCols, savedRows = CONFIG.gridRows;
    CONFIG.gridCols = bagGridCols;
    CONFIG.gridRows = bagGridRows;
    const frag = document.createDocumentFragment();
    bagInventoryData.items.forEach(item => frag.appendChild(createItemElement(item, 'bag')));
    dropGrid.appendChild(frag);
...
function openBag(data) {
    ...
    const dropHint = document.getElementById('drop-hint');
    const dropIcon = document.getElementById('drop-icon');
    if (dropTitle) dropTitle.textContent = data.label || 'Bag';
    if (dropIcon) dropIcon.className = 'fa-solid fa-suitcase hex-inner-icon';
    if (dropHint) dropHint.classList.add('hidden');
    const seg = getBagWeightSegEl();
    const txt = getBagWeightTextEl();
...
function closeBagMode() {
    ...
    const dropHint = document.getElementById('drop-hint');
    const dropIcon = document.getElementById('drop-icon');
    if (dropTitle) dropTitle.textContent = 'Ground';
    if (dropIcon) dropIcon.className = 'fa-solid fa-arrow-down hex-inner-icon';
    ...
    if (typeof renderGroundItems === 'function') renderGroundItems();
}
...
function openLootContainer(data) {
    ...
    data.items.forEach((item, index) => {
        ...
        el.addEventListener('dblclick', (e) => {
            ...
            if (el.dataset.pending === 'true' || el.dataset.taken === 'true') return;
            const serverIndex = item.index != null ? Number(item.index) : (Number(index) + 1);
            if (!serverIndex || serverIndex < 1) return;
            el.dataset.pending = 'true';
            el.dataset.lootIndex = String(serverIndex);
            fetch('https://' + GetParentResourceName() + '/takeLootItem', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json; charset=UTF-8' },
                body: JSON.stringify({
                    containerId: containerData && containerData.containerId,
